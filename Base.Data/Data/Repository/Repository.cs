...
public class Repository<T> : IRepository<T> where T : class
{
    ...
    null)
    {
        ...
        // Se aplica el filtro si se utiliza
        if (filter != null)
        {
        ...
        if (orderBy != null)
            {
                // Se ejecuta la función de ordenamiento y se convierte la consulta en
                //una lista
                return orderBy(query).ToList();
            }
            // Si no se proporciona ordenamiento, simplemente se convierte la consulta
            //en una lista
return query.ToList();
        }
